...
test "floyd_warshall basic path finding" {
    var fw = FloydWarshall.init(std.testing.allocator);
    defer fw.deinit();
    ...
    try std.testing.expect(fw.hasPath(0, 2));
    try std.testing.expectEqual(@as(u64, 2), fw.value(0, 2));
}

test "floyd_warshall with entity mapping" {
    var fw = FloydWarshall.init(std.testing.allocator);
    defer fw.deinit();
    ...
    try std.testing.expect(fw.hasPathWithMapping(100, 300));
    try std.testing.expectEqual(@as(u64, 2), fw.valueWithMapping(100, 300));
}
...
test "floyd_warshall path reconstruction" {
    ...
    try fw.setPathWithMapping(&path, 10, 40);
